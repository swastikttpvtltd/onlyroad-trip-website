...
export default function HeroSearch(){
 ...
 return <div className="relative overflow-visible rounded-[32px] border border-white/20 bg-white/10 p-5 backdrop-blur-3xl shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
  ...
  <div className="relative z-10 grid grid-cols-1 gap-5 lg:grid-cols-4">
   ...
   <div className="rounded-3xl border border-white/20 bg-white/10 p-5 backdrop-blur-xl transition-all duration-300 hover:border-cyan-400 hover:bg-white/15 hover:shadow-lg">
    <div className="mb-3 flex items-center gap-2"><Calendar className="h-5 w-5 text-cyan-300"/><span className="text-xs font-semibold uppercase tracking-widest text-white/70">Travel Date</span></div>
    <input type="date" min={todayIso()} value={travelDate} onChange={e=>setTravelDate(e.target.value)} className="w-full bg-transparent text-lg font-semibold text-white outline-none"/>
   </div>
   <div className="rounded-3xl border border-white/20 bg-white/10 p-5 backdrop-blur-xl transition-all duration-300 hover:border-cyan-400 hover:bg-white/15 hover:shadow-lg">
    <div className="mb-3 flex items-center gap-2"><Users className="h-5 w-5 text-cyan-300"/><span className="text-xs font-semibold uppercase tracking-widest text-white/70">Travellers</span></div>
    <div className="flex items-center justify-between"><button type="button" onClick={()=>setTravellers(p=>Math.max(1,p-1))} className="flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-white/15 text-xl font-bold text-white">−</button><span className="text-2xl font-bold text-white">{travellers}</span><button type="button" onClick={()=>setTravellers(p=>p+1)} className="flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-white/15 text-xl font-bold text-white">+</button></div>
   </div>
   <button type="button" onClick={handleSearch} className="group flex items-center justify-center gap-3 rounded-3xl border border-cyan-400/40 bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-600 px-6 py-5 text-lg font-bold text-white shadow-[0_15px_40px_rgba(6,182,212,0.35)] transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02]"><Search className="h-5 w-5"/>Search Tours</button>
  </div>
 </div>;
}
