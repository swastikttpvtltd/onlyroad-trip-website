  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-[0_8px_28px_rgba(15,23,42,0.07)] transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_16px_38px_rgba(15,23,42,0.12)]">
      ...
      <div className="flex flex-1 flex-col px-4 py-4">
        ...
        <div className="mt-3 flex items-center justify-between text-[12px]"><Link href={`https://www.google.com/maps/search/?api=1&query=${mapQuery}`} target="_blank" rel="noreferrer" className="font-medium text-slate-700 underline underline-offset-2 hover:text-blue-700">{t("viewMap")}</Link><Link href={`/packages/${pkg.slug}#highlights`} className="font-medium text-slate-700 underline underline-offset-2 hover:text-blue-700">{t("highlights")}</Link></div>
        <div className="mt-4 border-t border-slate-100 pt-3"><p className="mb-2 text-sm font-semibold text-slate-800">{t("tourIncludes")}</p><div className="flex items-center gap-4 text-blue-700"><span title={t("accommodation")} aria-label={t("accommodation")} className="text-base">▰</span>{hasMeals && <Utensils className="h-4 w-4" aria-label={t("mealsIncluded")} />}{hasSightseeing && <Camera className="h-4 w-4" aria-label={t("sightseeingIncluded")} />}{hasTransport && <Bus className="h-4 w-4" aria-label={t("transportIncluded")} />}</div></div>
        <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3"><p className="text-[11px] font-medium uppercase tracking-wide text-slate-500">{isGroup ? t("groupPrice") : t("allInclusive")}</p><div className="mt-1 flex items-end justify-between gap-3"><div>{displayPrice !== null ? <><h2 className="text-[25px] font-extrabold leading-none text-slate-950">₹{displayPrice.toLocaleString("en-IN")}<span className="text-lg">*</span></h2><p className="mt-1 text-[11px] text-slate-500">{t("perPerson")}</p></> : <><h2 className="text-[22px] font-extrabold leading-none text-slate-950">{t("rateSoon")}</h2><p className="mt-1 text-[11px] text-slate-500">{t("quad")}</p></>}</div><div className="text-right text-[11px] text-blue-700"><p className="font-bold">{t("flexiblePayment")}</p><p className="text-slate-500">{t("optionsAvailable")}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><Link href={`/packages/${pkg.slug}`} className="inline-flex h-10 items-center justify-center rounded-lg border border-blue-700 px-3 text-sm font-bold text-blue-800 transition hover:bg-blue-50">{t("viewTour")}</Link><Link href={`/book/${pkg.slug}`} className="inline-flex h-10 items-center justify-center rounded-lg bg-blue-800 px-3 text-sm font-bold text-white shadow-sm transition hover:bg-blue-900">{t("bookOnline")}</Link></div></div>
        <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-[12px]"><Link href="https://wa.me/?text=I%20want%20to%20enquire%20about%20this%20tour" target="_blank" rel="noreferrer" className="font-semibold text-emerald-600 hover:underline">{t("whatsapp")}</Link><Link href={`/packages/${pkg.slug}#itinerary`} className="inline-flex items-center gap-1 text-slate-600 hover:text-blue-700">{t("getItinerary")} <ArrowRight className="h-3.5 w-3.5" /></Link></div>
