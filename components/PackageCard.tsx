    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-[0_8px_28px_rgba(15,23,42,0.07)] transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_16px_38px_rgba(15,23,42,0.12)]">
      <div className="relative h-[220px] w-full shrink-0 overflow-hidden bg-slate-100 sm:h-[230px]">
        ...
        <div className="absolute left-3 right-3 top-3 flex items-start justify-between gap-2">
          <span className="inline-flex max-w-[78%] rounded-full bg-blue-800 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wide text-white shadow-md">{pkg.category}</span>
          <button type="button" aria-label="Add to Wishlist" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 shadow-md transition hover:scale-105 hover:text-blue-800"><Heart className="h-5 w-5 stroke-[1.8]" /></button>
        </div>
      ...
      <div className="flex flex-1 flex-col px-4 py-4">
        ...
        <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3"><p className="text-[11px] font-medium uppercase tracking-wide text-slate-500">All inclusive price starts</p><div className="mt-1 flex items-end justify-between gap-3"><div><h2 className="text-[25px] font-extrabold leading-none text-slate-950">₹{pkg.price.toLocaleString("en-IN")}<span className="text-lg">*</span></h2><p className="mt-1 text-[11px] text-slate-500">Per Person</p></div><div className="text-right text-[11px] text-blue-700"><p className="font-bold">Flexible payment</p><p className="text-slate-500">options available</p></div></div>
          <div className="mt-3 grid grid-cols-2 gap-2"><Link href={`/packages/${pkg.slug}`} className="inline-flex h-10 items-center justify-center rounded-lg border border-blue-700 px-3 text-sm font-bold text-blue-800 transition hover:bg-blue-50">View Tour</Link><Link href={`/packages/${pkg.slug}#booking`} className="inline-flex h-10 items-center justify-center rounded-lg bg-blue-800 px-3 text-sm font-bold text-white shadow-sm transition hover:bg-blue-900">Book Online</Link></div>
        </div>
