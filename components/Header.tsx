...
export default function Header() {
  ...
  const navLinkClass = "group relative text-[15px] font-medium text-white transition-all duration-300 hover:text-cyan-300";

  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ease-in-out ${scrolled ? "top-3 mx-auto max-w-7xl rounded-2xl border border-white/20 bg-slate-950/80 backdrop-blur-3xl shadow-[0_10px_40px_rgba(0,0,0,0.35)]" : "border-b border-white/10 bg-slate-950/70 backdrop-blur-2xl"}`}>
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          <Link href="/" className={navLinkClass}>Home<span className="absolute -bottom-1 left-0 h-[2px] w-0 bg-cyan-400 transition-all duration-300 group-hover:w-full" /></Link>
          <Link href="/about" className={navLinkClass}>About Us<span className="absolute -bottom-1 left-0 h-[2px] w-0 bg-cyan-400 transition-all duration-300 group-hover:w-full" /></Link>
          ...
          <div className="group/experiences relative flex h-[72px] items-center">
            <Link href="/packages" className={`${navLinkClass} flex items-center gap-1.5`}>
              ...
              <span className="absolute -bottom-1 left-0 h-[2px] w-0 bg-cyan-400 transition-all duration-300 group-hover/experiences:w-full" />
            </Link>
            ...
            <div className="invisible absolute left-1/2 top-[66px] w-[760px] -translate-x-1/2 translate-y-2 opacity-0 transition-all duration-200 group-hover/experiences:visible group-hover/experiences:translate-y-0 group-hover/experiences:opacity-100">
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
                <div className="border-b border-slate-100 bg-gradient-to-r from-blue-950 to-blue-800 px-6 py-4 text-white">
                  <p className="text-lg font-bold">Explore by Experience</p>
                  <p className="mt-0.5 text-xs text-blue-100">Choose the way you want to travel</p>
                </div>
                ...
                <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50 px-6 py-3">
                  ...
                  <Link href="/packages" className="text-sm font-bold text-blue-800 hover:text-orange-600">View All Packages →</Link>
                </div>
              </div>
            </div>
          </div>

          <Link href="/packages" className={navLinkClass}>Packages<span className="absolute -bottom-1 left-0 h-[2px] w-0 bg-cyan-400 transition-all duration-300 group-hover:w-full" /></Link>
          <Link href="/corporate-travel" className={navLinkClass}>Corporate Travel<span className="absolute -bottom-1 left-0 h-[2px] w-0 bg-cyan-400 transition-all duration-300 group-hover:w-full" /></Link>
          <Link href="/contact" className={navLinkClass}>Contact<span className="absolute -bottom-1 left-0 h-[2px] w-0 bg-cyan-400 transition-all duration-300 group-hover:w-full" /></Link>
        </nav>

        <Link href="/contact" className="rounded-full border border-white/20 bg-white/10 backdrop-blur-md px-6 py-2.5 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:scale-105 hover:border-cyan-400 hover:bg-cyan-500 hover:shadow-cyan-500/40">Plan Your Trip</Link>
      </div>
    </header>
  );
}