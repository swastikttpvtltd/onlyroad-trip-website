...
const packageStates = [
  "Andaman & Nicobar Islands", "Andhra Pradesh", "Assam", "Goa", "Gujarat", "Himachal Pradesh", "Jammu & Kashmir", "Karnataka", "Kerala", "Ladakh", "Madhya Pradesh", "Maharashtra", "Meghalaya", "Punjab", "Rajasthan", "Sikkim", "Tamil Nadu", "Uttar Pradesh", "Uttarakhand", "West Bengal",
];
  ...
  useEffect(() => {
    ...
    const detect = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const y = Math.max(1, Math.min(innerHeight - 1, Math.round(r.top + r.height / 2)));
  ...
  return (
    <header ref={headerRef} className="fixed inset-x-0 top-3 z-50 px-3 sm:px-4">
      <div className={`relative mx-auto max-w-7xl rounded-2xl border shadow-[0_10px_35px_rgba(15,23,42,0.16)] backdrop-blur-xl transition-all duration-300 ${overLight ? "border-slate-300/80 bg-white/65" : "border-white/45 bg-slate-950/35"}`}>
        <div className="flex h-[60px] items-center justify-between px-3 sm:h-[68px] sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            ...
            <div className="group/packages relative flex h-[68px] items-center">
              <Link href="/packages" className={`${nav} flex items-center gap-1.5`}>{t("packages")} ⌄</Link>
        ...
        {mobileOpen && <div className="border-t border-slate-200 bg-white p-3 lg:hidden"><div className="grid gap-1">{[[t("home"), "/"], [t("about"), "/about"], [t("packages"), "/packages"], [t("corporateTravel"), "/corporate-travel"], [t("contact"), "/contact"]].map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileOpen(false)} className="rounded-xl px-4 py-3 text-sm font-bold text-slate-800 hover:bg-blue-50 hover:text-blue-700">{label}</Link>)}<Link href="/plan-your-trip" onClick={() => setMobileOpen(false)} className="mt-2 rounded-xl bg-blue-700 px-4 py-3 text-center text-sm font-extrabold text-white">{t("planYourTrip")}</Link></div></div>}
