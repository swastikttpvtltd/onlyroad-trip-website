...
type Props = { packageTitle: string; packageId?: string };
...
function returnDate(value: string) {
  const d = new Date(`${value}T12:00:00`);
  d.setDate(d.getDate() + 2);
  return d.toLocaleDateString("en-IN", { weekday: "short", day: "2-digit", month: "short", year: "numeric" });
}

export default function GroupBookingForm({ packageTitle, packageId }: Props) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/payment-link", {
        ...
        body: JSON.stringify({
          amount: advance,
          purpose: `${packageTitle} | ${sharing} sharing | ${travelDate} | ${travellers} travellers`,
          customer_name: name,
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-3xl bg-white p-6 shadow-xl md:p-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.22em] text-blue-700">Group Tour Booking</p>
              <h2 className="mt-2 text-3xl font-extrabold text-slate-950">Select Your Trip</h2>
              <p className="mt-2 text-sm font-medium text-slate-500">{packageTitle}{packageId ? ` • ${packageId}` : ""}</p>
            </div>
          ...
          <div className="mt-7 rounded-2xl border border-slate-200 overflow-hidden">
            ...
            <table className="w-full border-collapse">
              <thead><tr className="bg-amber-400 text-left"><th className="p-4 font-extrabold">Room Sharing</th><th className="p-4 text-right font-extrabold">Cost (per person)</th></tr></thead>
              <tbody>
                {(["quad","triple","double"] as Sharing[]).map((type) => {
                  const label = type === "quad" ? "Quad Sharing" : type === "triple" ? "Triple Sharing" : "Double Sharing";
                  return <tr key={type} className={`border-t border-slate-200 ${sharing === type ? "bg-blue-50" : "bg-white"}`}><td className="p-4 font-bold">{label}</td><td className="p-4 text-right font-extrabold">₹{rates[type].toLocaleString("en-IN")}/-</td></tr>;
                })}
              </tbody>
            </table>
            <div className="border-t bg-slate-50 px-5 py-3 text-xs font-semibold text-slate-600">Fixed per-person rate. The same per-person rate applies whether you book 1 traveller, 2 travellers or more. GST @ 5% is already included.</div>
          </div>

          <div className="mt-7 rounded-2xl border border-slate-200 p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div><h3 className="text-xl font-extrabold text-slate-950">Departure Slots</h3><p className="mt-1 text-sm text-slate-500">Every Friday departure • 2 Nights / 3 Days • slots shown through January 2028</p></div>
              <select value={selectedMonth} onChange={(e) => { const first = FRIDAYS.find((date) => date.startsWith(e.target.value)); if (first) setTravelDate(first); }} className="rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-bold text-slate-800">
                {months.map((month) => <option key={month} value={month}>{new Date(`${month}-01T12:00:00`).toLocaleDateString("en-IN", { month: "long", year: "numeric" })}</option>)}
              </select>
            </div>
            <div className="mt-4 overflow-x-auto">
              <table className="w-full min-w-[620px] border-collapse text-sm">
                <thead><tr className="bg-slate-100 text-left"><th className="border p-3">Departure</th><th className="border p-3">Return</th><th className="border p-3">Status</th><th className="border p-3 text-center">Select</th></tr></thead>
                <tbody>{visibleSlots.map((date) => <tr key={date} className={travelDate === date ? "bg-blue-50" : "bg-white"}><td className="border p-3 font-bold">{formatDate(date)}</td><td className="border p-3 font-semibold">{returnDate(date)}</td><td className="border p-3"><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">Available</span></td><td className="border p-3 text-center"><input type="radio" name="departure" checked={travelDate === date} onChange={() => setTravelDate(date)} className="h-5 w-5 accent-blue-700" aria-label={`Select ${formatDate(date)}`} /></td></tr>)}</tbody>
              </table>
            </div>
          </div>
          ...
          <form onSubmit={submit} className="mt-7 space-y-5">
            <div className="grid gap-5 md:grid-cols-2">
              <Field label="Selected Departure"><div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 font-extrabold text-blue-950">{formatDate(travelDate)} → {returnDate(travelDate)}</div></Field>
              ...
              <Field label="Travellers"><input required type="number" min={1} value={travellers} onChange={(e) => setTravellers(Math.max(1, Number(e.target.value) || 1))} className="input" /></Field>
        ...
        <aside className="xl:sticky xl:top-28">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
            <div className="bg-slate-950 px-5 py-4 text-xl font-extrabold text-white">Booking Summary</div>
            <div className="space-y-4 p-5">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400">{packageTitle}</p>
              <div className="rounded-xl bg-slate-50 p-4 text-sm"><div className="flex justify-between gap-4"><span>Departure</span><b>{formatDate(travelDate)}</b></div><div className="mt-2 flex justify-between gap-4"><span>Return</span><b>{returnDate(travelDate)}</b></div><div className="mt-2 flex justify-between gap-4"><span>Sharing</span><b>{sharing === "quad" ? "Quad" : sharing === "triple" ? "Triple" : "Double"}</b></div><div className="mt-2 flex justify-between gap-4"><span>Travellers</span><b>{travellers}</b></div></div>
              <Row label={`₹${perPerson.toLocaleString("en-IN")} × ${travellers} travellers`} value={total} />
              ...
              <p className="text-xs leading-5 text-slate-500">Slot booking is subject to final availability and confirmation. Payment is processed securely through the configured payment gateway.</p>
            </div>
          </div>
        </aside>
      </div>
