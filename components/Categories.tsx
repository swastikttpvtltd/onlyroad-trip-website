    ...
    <section className="bg-slate-50 py-10 md:py-12">
      <div className="mx-auto max-w-[1500px] px-5 lg:px-8">
        <div className="mx-auto mb-6 max-w-3xl text-center">
          <span className="rounded-full bg-blue-100 px-3.5 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-blue-800">
            Travel Categories
          </span>
        ...
        <div className="relative px-8 md:px-10">
            ...
            {categories.map((category) => {
              ...
              return (
                <SwiperSlide key={category.id} className="!h-auto">
                  <div className="group flex h-full min-h-[175px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_25px_rgba(15,23,42,0.05)] transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_14px_35px_rgba(15,23,42,0.10)]">
                    <div className="mb-4 flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-800 text-white transition duration-300 group-hover:scale-105">
                      <Icon size={24} />
                    </div>
                    <h3 className="text-lg font-bold text-slate-900">{category.title}</h3>
                    <p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-600">{category.description}</p>
                    <Link href={`/packages?theme=${encodeURIComponent(category.theme)}`} className="mt-auto inline-flex items-center gap-2 pt-3 text-xs font-semibold text-blue-800 transition group-hover:gap-3">Explore <ArrowRight size={15} /></Link>
                  </div>
                </SwiperSlide>
              );
            })}
          ...
          <button type="button" className="categories-next absolute right-0 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 shadow-md transition hover:scale-105 hover:bg-slate-50" aria-label="Next travel category"><ChevronRight size={20} /></button>
        </div>
      </div>
    </section>
  );
