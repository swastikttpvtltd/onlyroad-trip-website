...
export default function Categories(){
 const t=useTranslations("home"); const c=useTranslations("categories");
 ...
 <div className="relative px-8 md:px-10"><button type="button" className="categories-prev absolute left-0 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 shadow-md" aria-label={c("previous")}><ChevronLeft size={20}/></button>
 ...
 {categories.map(category=>{const Icon=icons[category.icon as keyof typeof icons];return <SwiperSlide key={category.id} className="!h-auto"><div className="group flex h-full min-h-[175px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_25px_rgba(15,23,42,0.05)]"><div className="mb-4 flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-800 text-white"><Icon size={24}/></div><h3 className="text-lg font-bold text-slate-900">{c(category.theme === "pilgrimage" ? "pilgrimage" : category.theme === "wildlife" ? "wildlife" : category.theme === "women" ? "women" : category.theme === "corporate" ? "corporate" : "family")}</h3><p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-600">{c(category.theme === "pilgrimage" ? "pilgrimageDesc" : category.theme === "wildlife" ? "wildlifeDesc" : category.theme === "women" ? "womenDesc" : category.theme === "corporate" ? "corporateDesc" : "familyDesc")}</p><Link href={`/packages?theme=${encodeURIComponent(category.theme)}`} className="mt-auto inline-flex items-center gap-2 pt-3 text-xs font-semibold text-blue-800">{t("explore")} <ArrowRight size={15}/></Link></div></SwiperSlide>})}
 </Swiper><button type="button" className="categories-next absolute right-0 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 shadow-md" aria-label={c("next")}><ChevronRight size={20}/></button></div></div></section>;
}
