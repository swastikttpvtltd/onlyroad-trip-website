...
function firstBookableDate(isGroup: boolean, duration?: string) {
  const minimum = todayPlusDays(7);
  const candidates = isGroup ? buildFridayDates(minimum) : [minimum, ...Array.from({ length: 370 }, (_, index) => isoDate(addDays(minimum, index + 1)))];
  return candidates.find((date) => getDepartureDecision(date, duration).bookable) ?? minimum;
}
...
function PrintItinerary({ pkg, finalTitle, finalDuration, finalDestination, displayPrice, sharingRates }: { pkg?: Props["pkg"]; finalTitle: string; finalDuration: string; finalDestination: string; displayPrice: number; sharingRates: SharingRate[] }) {
  ...
  return (
    <div className="itinerary-print-sheet">
      ...
      <section className="itinerary-print-section"><h2>Terms & Conditions</h2><ol>{terms.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ol></section>
...
export default function BookingSummaryCard({ pkg, slug, title, price, duration, destination }: Props) {
  ...
  const decision = getDepartureDecision(selectedDate, finalDuration);
  const arrivalDate = arrivalDateFor(selectedDate, finalDuration);
  const whatsappText = encodeURIComponent(`Hi, I want details for ${finalTitle}. Package: ${finalSlug}. Departure: ${selectedDate}. Arrival: ${arrivalDate}.`);
  const emailSubject = encodeURIComponent(`${decision.bookable ? "Booking" : "Enquiry"} for ${finalTitle} - ${selectedDate}`);
  const emailBody = encodeURIComponent(`Hi Only Road Trip,\n\nPackage: ${finalTitle}\nPackage ID: ${pkg?.packageId ?? ""}\nDeparture Date: ${formatDate(selectedDate)}\nArrival Date: ${formatDate(arrivalDate)}\nStatus: ${decision.bookable ? "Online booking requested" : "Enquiry only"}\nReason: ${decision.reason}`);
  const enquiryHref = `mailto:info@onlyroadtrip.com?subject=${emailSubject}&body=${emailBody}`;

  useEffect(() => setMounted(true), []);

  const downloadItinerary = () => { setMessage("Your focused itinerary document is ready. Choose Save as PDF in the print dialog."); window.print(); };
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><h3 className="text-lg font-bold text-slate-900">Plan Your Adventure</h3><div className="mt-4 grid grid-cols-3 gap-2 text-center"><a href={enquiryHref} className="rounded-xl p-2"><Mail className="mx-auto text-slate-500" size={22}/><span className="mt-2 block text-xs font-bold text-slate-800">Enquire Now</span></a><a href={enquiryHref} className="rounded-xl p-2"><Mail className="mx-auto text-slate-500" size={22}/><span className="mt-2 block text-xs font-bold text-slate-800">Email Itinerary</span></a><a target="_blank" rel="noopener noreferrer" href={`https://wa.me/919211796168?text=${whatsappText}`} className="rounded-xl p-2"><MessageCircle className="mx-auto text-slate-500" size={22}/><span className="mt-2 block text-xs font-bold text-slate-800">Send Itinerary</span></a></div><div className="mt-4 flex items-center gap-2 border-t border-slate-100 pt-4 text-xs text-slate-500"><Users size={15}/> Destination: <b className="text-slate-700">{finalDestination}</b></div></div>
      </div>
