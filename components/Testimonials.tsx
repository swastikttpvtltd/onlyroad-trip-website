    ...
    <section className="bg-white py-10 md:py-12">
      <div className="mx-auto max-w-7xl px-5">
        <div className="mb-7 text-center">
          <span className="rounded-full bg-blue-100 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-blue-800">Testimonials</span>
          <h2 className="mt-3 text-3xl font-extrabold text-slate-900 md:text-4xl">What Our Travelers Say</h2>
          <p className="mx-auto mt-2 max-w-3xl text-sm text-slate-600 md:text-base">Thousands of happy travelers trust Only Road Trip for unforgettable journeys across India.</p>
        </div>

        <Swiper modules={[Autoplay, Pagination, Navigation]} spaceBetween={18} loop={true} navigation pagination={{ clickable: true }} autoplay={{ delay: 4000, disableOnInteraction: false }} breakpoints={{ 0: { slidesPerView: 1 }, 768: { slidesPerView: 2 }, 1024: { slidesPerView: 3 } }} className="pb-12">
          {testimonials.map((item, index) => (
            <SwiperSlide key={index}>
              <div className="h-full rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_25px_rgba(15,23,42,0.05)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_14px_35px_rgba(15,23,42,0.10)]">
                <Quote className="mb-3 text-blue-700" size={28} />
                <div className="mb-3 flex">{[1, 2, 3, 4, 5].map((star) => <Star key={star} size={15} className="fill-amber-400 text-amber-400" />)}</div>
                <p className="min-h-[108px] text-sm leading-6 text-slate-600">&quot;{item.review}&quot;</p>
                <div className="mt-5 border-t border-slate-200 pt-4">
                  <h3 className="text-base font-bold text-slate-900">{item.name}</h3>
                  <p className="text-xs text-slate-500">📍 {item.location}</p>
                  <p className="mt-1.5 text-xs font-semibold text-blue-800">{item.tour}</p>
                  <div className="mt-2 inline-flex items-center rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-semibold text-emerald-700">✓ Verified Traveller</div>
                </div>
              </div>
            </SwiperSlide>
          ))}
