    ...
    <section className="bg-white py-10 md:py-12">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="mb-7 text-center md:mb-8">
          <span className="inline-flex rounded-full bg-blue-100 px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.18em] text-blue-800">Explore India</span>
          <h2 className="mt-3 text-3xl font-extrabold tracking-tight text-slate-900 md:text-4xl">Featured Packages</h2>
          <p className="mx-auto mt-2 max-w-2xl text-sm text-slate-600 md:text-base">Handpicked journeys from our latest travel packages.</p>
        </div>

        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {featuredPackages.map((pkg) => (
            <article key={pkg.slug} className="group overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-[0_7px_25px_rgba(15,23,42,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_14px_35px_rgba(15,23,42,0.13)]">
              <div className="relative aspect-[16/10] overflow-hidden bg-slate-100">
                <Image src={pkg.image} alt={pkg.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" loading="lazy" className="object-cover transition-transform duration-500 group-hover:scale-[1.04]" />
        ...
        <div className="mt-7 text-center"><Link href="/packages" className="inline-flex items-center gap-2 rounded-full border border-slate-300 px-5 py-2.5 text-sm font-semibold text-slate-800 transition hover:border-blue-600 hover:text-blue-800">View All Packages <ArrowRight size={16} /></Link></div>
      </div>
    </section>
  );
}
