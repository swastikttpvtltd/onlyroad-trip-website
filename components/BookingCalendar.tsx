    ...
    <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-[520px] text-[11px] leading-4 text-slate-500">
      ...
      <div className="mt-3 flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
        <span className="text-xs font-extrabold text-slate-800">{monthLabel(`${currentMonthKey}-01`)}</span>
        <span className="text-[9px] font-bold uppercase tracking-wide text-slate-400">India Calendar</span>
      </div>

      <div className="mt-2 grid grid-cols-7 gap-1 text-center text-[9px] font-extrabold uppercase tracking-wide text-slate-400">
        ...
        {days.map((date) => {
          ...
          return (
            <button
              ...
              className={`relative min-h-[40px] rounded-md border p-0.5 text-[11px] transition ${!inMonth ? "invisible" : ""} ${selected && enabled ? "border-blue-700 bg-blue-700 text-white shadow" : ""} ${selected && enquiryDate ? "border-rose-600 bg-rose-600 text-white shadow" : ""} ${!selected && enabled ? "border-slate-200 bg-white text-slate-800 hover:border-blue-400 hover:bg-blue-50" : ""} ${!enabled && !enquiryDate && inMonth ? "cursor-not-allowed border-slate-100 bg-slate-50 text-slate-300" : ""} ${enquiryDate ? "border-rose-200 bg-rose-50 text-rose-800 hover:border-rose-400" : ""}`}>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5 text-[9px] font-bold">
        ...
        <span className="rounded-full bg-rose-50 px-2 py-1 text-rose-700">● Holiday / Enquiry</span>
        <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-500">● Less than 7 days</span>
      </div>

      {monthHolidays.length > 0 && (
        <div className="mt-2 rounded-lg border border-rose-100 bg-rose-50 p-2.5">
          <p className="text-[9px] font-extrabold uppercase tracking-wide text-rose-600">Major / travel-impact holidays</p>
          <p className="mt-0.5 text-[10px] text-slate-600">{monthHolidays.map((holiday) => `${holiday.name} (${holiday.date.slice(-2)})`).join(" • ")}</p>
        </div>
      )}
    </div>
  );
