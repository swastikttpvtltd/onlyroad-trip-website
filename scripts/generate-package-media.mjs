...
async function main() {
  const files = (await walk(mediaRoot)).sort((a, b) => a.localeCompare(b));
  const grouped = {};
  ...
  console.log(`Package media manifest generated: ${files.length} image(s), with no per-package image-count or extension whitelist.`);
}
