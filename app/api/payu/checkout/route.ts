...
export async function POST(request: Request) {
  try {
    ...
    if (environment !== "test") return NextResponse.json({ error: "This checkout route is locked to PayU TEST mode." }, { status: 500 });
    if (!key || !salt) return NextResponse.json({ error: "PayU TEST key/salt are not configured for this local environment." }, { status: 500 });
    ...
    const hashString = [key, txnid, formattedAmount, productinfo, firstname, email, udf1, udf2, "", "", "", "", "", "", "", "", "", salt].join("|");
    const hash = await sha512(hashString);

    return NextResponse.json({
      actionUrl,
      fields: {
        key, txnid, amount: formattedAmount, productinfo, firstname, email, phone, surl, furl, hash,
        api_version: "7", udf1, udf2, udf3: "", udf4: "", udf5: "",
      },
    });
  } catch (err: any) {
