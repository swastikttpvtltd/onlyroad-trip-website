...
export async function POST(request: Request) {
  try {
    ...
    if (!clientId || !clientSecret) return NextResponse.json({ error: "Cashfree credentials are not configured on the server. Add CASHFREE_APP_ID and CASHFREE_SECRET_KEY to .env.local." }, { status: 500 });
    ...
    const payload = {
      ...
      customer_details: {
        ...
        customer_phone: String(body.customer_phone).replace(/\D/g, "").slice(-15),
      },
    ...
    const response = await fetch(endpoint, {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-version": "2026-01-01", "x-client-id": clientId, "x-client-secret": clientSecret },
      body: JSON.stringify(payload),
      cache: "no-store",
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) return NextResponse.json({ error: data?.message || data?.message_text || data?.error_description || "Cashfree could not create the payment link." }, { status: response.status });
    ...
    console.error("Cashfree payment link failed:", error);
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to create Cashfree payment link." }, { status: 500 });
  }
}
