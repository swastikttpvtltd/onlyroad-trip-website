...
function getBookingMeta(body: any, amount: number) {
  ...
  return { title, packageId: String(pkg?.packageId || ""), duration, sharing, date, returnDate: calculateReturnDate(date, duration), travellers, rate, total, advance: amount, balance: Math.max(0, total - amount) };
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!body.customer_name || !body.customer_email || !body.customer_phone || !body.purpose) return NextResponse.json({ error: "Customer name, email, mobile and payment purpose are required." }, { status: 400 });
    ...
    if (body.action !== "create_cashfree") {
      const params = new URLSearchParams({
        title: booking.title, packageId: booking.packageId, duration: booking.duration, date: booking.date, returnDate: booking.returnDate,
        sharing: booking.sharing, travellers: String(booking.travellers), rate: String(booking.rate), total: String(booking.total),
        advance: String(booking.advance), balance: String(booking.balance), name: String(body.customer_name).trim(), phone: String(body.customer_phone).trim(),
        email: String(body.customer_email).trim(), purpose: String(body.purpose).slice(0, 500),
      });
    ...
    if (!clientId || !clientSecret) return NextResponse.json({ error: "Cashfree API credentials are not configured on the server." }, { status: 500 });
    ...
    const bookingNumber = String(body.bookingNumber || `ORT-${Date.now().toString(36).slice(-8).toUpperCase()}`);
    ...
    const payload = {
      ...
      link_amount: amount,
      link_currency: "INR",
      link_purpose: `${String(body.purpose).slice(0, 450)} | ${bookingNumber}`,
      customer_details: { customer_name: String(body.customer_name).trim(), customer_email: String(body.customer_email).trim(), customer_phone: String(body.customer_phone).trim() },
