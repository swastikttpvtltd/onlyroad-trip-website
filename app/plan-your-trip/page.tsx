      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1.1fr_.9fr] lg:py-20">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-7 shadow-[0_20px_60px_rgba(15,23,42,0.10)] sm:p-10">
          ...
          <form className="mt-8 grid gap-5 sm:grid-cols-2">
            <input required placeholder="Full Name" onFocus={() => setShowDestinationResults(false)} className="rounded-xl border border-slate-300 bg-slate-50 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" />
            <input required type="tel" placeholder="Mobile Number" onFocus={() => setShowDestinationResults(false)} className="rounded-xl border border-slate-300 bg-slate-50 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" />
            <input type="email" placeholder="Email Address" onFocus={() => setShowDestinationResults(false)} className="rounded-xl border border-slate-300 bg-slate-50 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" />
            ...
            <div ref={destinationRef} className="relative">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  setShowDestinationResults(value.trim().length > 0);
                }}
                onFocus={() => setShowDestinationResults(destination.trim().length > 0)}
                placeholder="Search destination, state or district"
                autoComplete="off"
                className="w-full rounded-xl border border-slate-300 bg-slate-50 py-3.5 pl-11 pr-4 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"
              />
                ...
                <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 max-h-64 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_20px_45px_rgba(15,23,42,0.18)]">
                  {filteredDestinations.map((item) => (
                    <button
                      ...
                      className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-slate-800 transition hover:bg-blue-50"
                    >
              ...
              {showDestinationResults && destination.trim() && filteredDestinations.length === 0 && (
                <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-500 shadow-[0_20px_45px_rgba(15,23,42,0.18)]">
                  No matching destination found. You can continue with your typed destination.
                </div>
              )}
            </div>

            <div className="relative">
              <CalendarDays className="pointer-events-none absolute left-4 top-4 z-10 text-blue-600" size={19} />
              <input
                ref={dateRef}
                type="date"
                min={today}
                onFocus={() => setShowDestinationResults(false)}
                className="w-full cursor-pointer rounded-xl border border-slate-300 bg-slate-50 py-3.5 pl-11 pr-12 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:right-4 [&::-webkit-calendar-picker-indicator]:h-0 [&::-webkit-calendar-picker-indicator]:w-0 [&::-webkit-calendar-picker-indicator]:opacity-0"
                aria-label="Travel date"
              />
              <button type="button" onClick={openCalendar} aria-label="Open travel date calendar" className="absolute inset-y-0 right-3 flex w-9 items-center justify-center rounded-lg text-blue-600 transition hover:bg-blue-100">
                <CalendarDays size={19} />
              </button>
            </div>

            <div className="relative" onFocus={() => setShowDestinationResults(false)}>
            ...
            <select onFocus={() => setShowDestinationResults(false)} className="rounded-xl border border-slate-300 bg-slate-50 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"><option>Travel Type</option><option>Family Holiday</option><option>Honeymoon</option><option>Pilgrimage</option><option>Road Trip</option><option>Group Travel</option><option>Corporate Travel</option></select>
            <select onFocus={() => setShowDestinationResults(false)} className="rounded-xl border border-slate-300 bg-slate-50 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"><option>Approx. Budget</option><option>Under ₹25,000</option><option>₹25,000 – ₹50,000</option><option>₹50,000 – ₹1,00,000</option><option>₹1,00,000+</option></select>
            <textarea rows={5} placeholder="Requirements / Message" onFocus={() => setShowDestinationResults(false)} className="sm:col-span-2 rounded-xl border border-slate-300 bg-slate-50 px-4 py-3.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" />
        ...
        <div className="space-y-6">
          <div className="rounded-[2rem] border border-slate-200 bg-slate-50 p-7 sm:p-9">
            ...
            <div className="mt-7 space-y-5">
              {["Customised itineraries instead of one-size-fits-all plans", "Comfort-focused transport and practical route planning", "Clear package inclusions, exclusions and booking guidance", "Personal support before and during your journey"].map(item=><div key={item} className="flex gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-blue-700" size={21}/><p className="font-semibold leading-6 text-slate-700">{item}</p></div>)}
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            <div className="rounded-2xl bg-blue-50 p-5"><MapPin className="text-blue-700"/><p className="mt-3 font-extrabold">Destination-led planning</p><p className="mt-1 text-sm leading-6 text-slate-600">From mountains and pilgrimage circuits to family holidays, we plan around the destination and your pace.</p></div>
            <div className="rounded-2xl bg-slate-50 p-5"><Car className="text-blue-700"/><p className="mt-3 font-extrabold">Comfortable travel</p><p className="mt-1 text-sm leading-6 text-slate-600">Transport and sightseeing can be planned around your final group size and requirements.</p></div>
          </div>
        </div>
      </section>

      <section className="bg-slate-50 px-6 py-16 sm:py-20">
        <div className="mx-auto max-w-7xl"><div className="text-center"><p className="text-sm font-bold uppercase tracking-[0.18em] text-blue-700">How it works</p><h2 className="mt-2 text-3xl font-extrabold sm:text-4xl">Three simple steps to your holiday</h2></div><div className="mt-10 grid gap-5 md:grid-cols-3">{steps.map(([num,title,text])=><div key={num} className="rounded-[1.75rem] border border-slate-200 bg-white p-7 shadow-sm"><span className="text-sm font-extrabold text-blue-700">{num}</span><h3 className="mt-4 text-xl font-extrabold">{title}</h3><p className="mt-2 leading-7 text-slate-600">{text}</p></div>)}</div></div>
      </section>
