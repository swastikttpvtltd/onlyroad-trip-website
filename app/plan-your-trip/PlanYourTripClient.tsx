...
export default function PlanYourTripClient() {
  ...
  return (
    <>
      ...
      <section className="px-6 py-16"><div className="mx-auto max-w-4xl rounded-[2rem] bg-gradient-to-r from-blue-700 to-blue-900 p-8 text-center text-white shadow-2xl sm:p-12"><h2 className="text-3xl font-extrabold sm:text-4xl">Ready to start planning?</h2><p className="mx-auto mt-3 max-w-2xl leading-7 text-blue-100">If you would rather speak to us directly, our travel team is just a call or WhatsApp message away.</p><div className="mt-7 flex flex-wrap justify-center gap-3"><a href="https://wa.me/919211796168" target="_blank" rel="noopener noreferrer" aria-label="Chat with Only Road Trip on WhatsApp" className="inline-flex items-center gap-2 rounded-full border border-white bg-white px-6 py-3 font-extrabold !text-blue-800 shadow-sm hover:bg-slate-50"><MessageCircle size={18} className="!text-blue-800"/><span className="!text-blue-800">WhatsApp Us</span></a><a href="tel:+919211796168" className="rounded-full border border-white/40 bg-white/10 px-6 py-3 font-extrabold text-white">Call +91 92117 96168</a></div><Link href="/contact" className="mt-6 inline-block text-sm font-bold text-blue-100 underline underline-offset-4">Need general support? Contact us →</Link></div></section>
    </>
  );
}
