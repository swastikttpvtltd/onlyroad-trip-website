...
export default function PaymentSuccessPage() {
  ...
  function printConfirmation() {
    ...
    const makeRows = (items: string[][]) => items.map(([label, value]) => `
...
.header{border-bottom:2px solid #153e75;padding-bottom:12px;margin-bottom:16px}
.brand{color:#153e75;font-size:9pt;font-weight:800;letter-spacing:.18em}
h1{margin:5px 0 2px;font-size:23pt;line-height:1.15;color:#111827}
.muted{color:#64748b;font-size:9.5pt}
.refgrid,.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
.ref{border:1px solid #bfdbfe;border-radius:8px;background:#eff6ff;padding:10px}
.box{border:1px solid #cbd5e1;border-radius:8px;padding:9px;break-inside:avoid;min-height:45px}
.label{font-size:7.5pt;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:#64748b}
.ref .label{color:#2563eb}
.value{margin-top:3px;font-size:10.5pt;font-weight:800;color:#111827;word-break:break-word}
.ref .value{font-size:12pt;color:#172554}
.section{margin-top:17px}
h2{margin:0 0 8px;padding-left:8px;border-left:4px solid #153e75;color:#153e75;font-size:13.5pt}
.success{margin-top:16px;padding:11px;border:1px solid #a7f3d0;border-radius:8px;background:#ecfdf5;color:#065f46}
.footer{margin-top:20px;padding-top:9px;border-top:1px solid #cbd5e1;text-align:center;color:#64748b;font-size:8pt}
...
<div class="sheet">
  ...
  <div class="section"><h2>Traveller Details</h2><div class="grid">${makeRows(customerRows)}</div></div>
...
<script>window.onload=function(){setTimeout(function(){window.focus();window.print()},350)};window.onafterprint=function(){setTimeout(function(){window.close()},250)};<\/script>
</body></html>`);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section className="relative overflow-hidden bg-gradient-to-r from-slate-950 via-blue-950 to-slate-900 pt-32 pb-16 text-white sm:pt-36">
        ...
        <div className="relative mx-auto max-w-7xl px-6 text-center">
          <p className="text-sm font-semibold text-blue-200">Only Road Trip › Payment Successful</p>
      ...
      <main className="relative mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_20px_70px_rgba(15,23,42,0.10)] sm:p-10">
          <div className="text-center">
            <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full border-8 border-emerald-100 bg-emerald-50 text-emerald-600"><CheckCircle2 size={52} /></div>
          ...
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <section>
              <h3 className="flex items-center gap-2 text-xl font-extrabold"><MapPin className="text-blue-700" size={22} /> Trip Details</h3>
            ...
            <section>
              <h3 className="flex items-center gap-2 text-xl font-extrabold"><UserRound className="text-blue-700" size={22} /> Traveller Details</h3>
          ...
          <div className="mt-8 flex items-start gap-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-5">
            ...
            <div><p className="font-extrabold text-emerald-900">Payment received securely via {gatewayName}</p><p className="mt-1 text-sm leading-6 text-emerald-800">Keep your Booking Number and Transaction ID for future communication.</p></div>
          </div>

          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <button type="button" onClick={printConfirmation} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-700 px-6 py-3 font-extrabold text-white hover:bg-blue-800"><Download size={18} /> Save / Print Confirmation</button>
