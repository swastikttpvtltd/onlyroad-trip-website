import BookingForm from "@/components/BookingFormV2";
import GroupBookingForm from "@/components/GroupBookingFormV2";
...
export default async function BookPackagePage({ params }: Props) {
  ...
  return <main className="min-h-screen bg-slate-50 px-5 py-24 text-slate-800"><div className="mx-auto max-w-6xl">
    ...
    {isGroup ? <GroupBookingForm packageTitle={pkg.title} packageId={pkg.packageId} packageDuration={pkg.duration} /> : <BookingForm packageTitle={pkg.title} packageId={pkg.packageId} groupRates={pkg.groupRates} packageDuration={pkg.duration} />}
  </div></main>;
}
