...
import GroupBookingOptions from "@/components/GroupBookingOptions";
...
export default async function BookPackagePage({params}:Props){
 ...
 const isGroupTour=/group tour/i.test(String(pkg.category??""))||/group tour/i.test(String(pkg.title??""));
 return <main className="min-h-screen bg-slate-50 px-5 py-24 text-slate-800"><div className="mx-auto max-w-5xl">
  ...
  {isGroupTour&&<GroupBookingOptions packageTitle={pkg.title}/>} 
