...
export default async function PackagesPage({ searchParams }: { searchParams: Promise<SearchParams> | SearchParams }) {
  ...
  return <main className="min-h-screen bg-slate-50">
    <section className="bg-gradient-to-br from-blue-950 via-slate-900 to-blue-950 pb-14 pt-32 text-white"><div className="mx-auto max-w-7xl px-6 lg:px-8"><div className="max-w-3xl space-y-4"><p className="text-sm font-semibold uppercase tracking-[0.3em] text-cyan-300">Only Road Trip Experiences</p><h1 className="text-4xl font-bold tracking-tight md:text-5xl">{activeTheme ? `${activeTheme} Tour Experiences` : "Discover the best holiday packages across India"}</h1><p className="text-lg leading-8 text-slate-200">{activeTheme ? `Explore our ${activeTheme.toLowerCase()} journeys and choose the package that fits your travel style.` : "Choose journeys by travel style — spiritual, adventure, family, honeymoon, wildlife, beach, heritage, corporate and more."}</p></div></div></section>
    <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-[0.28em] text-cyan-600">{filteredPackages.length} Packages Available</p><h2 className="mt-2 text-3xl font-bold text-slate-900">{activeTheme ? `${activeTheme} Packages` : "All Tour Packages"}</h2></div>{hasFilters && <Link href="/packages" className="rounded-full border border-slate-300 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-cyan-500 hover:text-cyan-700">Clear All Filters</Link>}</div>
      <div className="grid gap-8 lg:grid-cols-[270px_minmax(0,1fr)]"><aside className="h-fit lg:sticky lg:top-24"><details className="group rounded-3xl border border-slate-200 bg-white shadow-sm lg:open" open><summary className="flex cursor-pointer list-none items-center justify-between px-5 py-4 lg:cursor-default [&::-webkit-details-marker]:hidden"><span className="text-base font-bold text-slate-900">Filter Your Trip</span><span className="text-xs font-semibold uppercase tracking-wider text-cyan-600 lg:hidden">Open</span></summary>
        <form action="/packages" method="get" className="space-y-6 border-t border-slate-100 px-5 pb-6 pt-5"><div><label htmlFor="package-search" className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">Search</label><input suppressHydrationWarning id="package-search" name="q" list="package-search-options" defaultValue={sp?.q ?? ""} placeholder="Package, destination or state" autoComplete="off" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm text-slate-800 outline-none transition focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-100" /><datalist id="package-search-options">{suggestions.map((item) => <option key={item} value={item} />)}</datalist><p className="mt-2 text-[11px] text-slate-400">Type “V” and choose Varanasi or another matching destination.</p></div>
          <div><label htmlFor="filter-state" className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">State / Region</label><select suppressHydrationWarning id="filter-state" name="state" defaultValue={activeState ?? ""} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-800 outline-none focus:border-cyan-500 focus:bg-white"><option value="">All States</option>{states.map((state) => <option key={state} value={state}>{state}</option>)}</select></div>
          <div><label htmlFor="filter-category" className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">Package Type</label><select suppressHydrationWarning id="filter-category" name="category" defaultValue={activeCategory ?? ""} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-800 outline-none focus:border-cyan-500 focus:bg-white"><option value="">All Package Types</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></div>
          <div><p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-500">Duration</p><div className="space-y-2.5">{durationOptions.map(([value, label]) => <label key={value} className="flex cursor-pointer items-center gap-3 text-sm text-slate-700"><input suppressHydrationWarning type="radio" name="duration" value={value} defaultChecked={duration === value} className="h-4 w-4 accent-cyan-600" />{label}</label>)}</div></div>
          <div><p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-500">Budget (₹)</p><div className="grid grid-cols-2 gap-2"><input suppressHydrationWarning name="minPrice" inputMode="numeric" defaultValue={sp?.minPrice ?? ""} placeholder="Min" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-cyan-500 focus:bg-white" /><input suppressHydrationWarning name="maxPrice" inputMode="numeric" defaultValue={sp?.maxPrice ?? ""} placeholder="Max" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-cyan-500 focus:bg-white" /></div></div>
          <div><p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-500">Travel Style</p><div className="grid gap-2.5">{themeOptions.map(([value, label]) => <label key={value} className="flex cursor-pointer items-center gap-3 text-sm text-slate-700"><input suppressHydrationWarning type="radio" name="theme" value={value} defaultChecked={activeTheme === value} className="h-4 w-4 accent-cyan-600" />{label}</label>)}</div></div>
          <div><label htmlFor="filter-sort" className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">Sort By</label><select suppressHydrationWarning id="filter-sort" name="sort" defaultValue={sort ?? ""} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-800 outline-none focus:border-cyan-500 focus:bg-white"><option value="">Recommended</option><option value="price-low">Price: Low to High</option><option value="price-high">Price: High to Low</option></select></div>
          <button type="submit" className="w-full rounded-xl bg-cyan-600 px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-cyan-700">Apply Filters</button><Link href="/packages" className="block text-center text-xs font-semibold text-slate-500 hover:text-cyan-700">Reset all filters</Link>
        </form></details></aside>
